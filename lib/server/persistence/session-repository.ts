...
export class SessionRepository {
  ...
  private migrate() {
    ...
    for(const [name,definition] of [["schedule_type","TEXT NOT NULL DEFAULT 'daily'"],["interval_value","INTEGER"],["interval_unit","TEXT"],["weekly_day","INTEGER"],["weekly_days","TEXT"],["occurrence_limit","INTEGER"]] as const)if(!scheduleColumns.has(name))this.database.exec("ALTER TABLE schedules ADD COLUMN "+name+" "+definition);
