...
export class SessionRepository {
  ...
  private migrate() {
    ...
    `);
    const addColumn=(table:string,name:string,definition:string)=>{try{this.database.exec("ALTER TABLE "+table+" ADD COLUMN "+name+" "+definition)}catch(error){if(!(error instanceof Error)||!/duplicate column name/i.test(error.message))throw error}};
    const scheduleColumns=new Set((this.database.prepare("PRAGMA table_info(schedules)").all() as {name:string}[]).map(column=>column.name));
    for(const [name,definition] of [["schedule_type","TEXT NOT NULL DEFAULT 'daily'"],["interval_value","INTEGER"],["interval_unit","TEXT"],["weekly_day","INTEGER"],["weekly_days","TEXT"],["occurrence_limit","INTEGER"]] as const)if(!scheduleColumns.has(name))addColumn("schedules",name,definition);
    this.database.exec("UPDATE schedules SET schedule_type='daily' WHERE schedule_type IS NULL OR schedule_type=''; UPDATE schedules SET occurrence_limit=repeat_days WHERE occurrence_limit IS NULL; UPDATE schedules SET schedule_type='weekly',weekly_days='[1,2,3,4,5]',weekly_day=1,occurrence_limit=NULL WHERE schedule_type='weekdays'; UPDATE schedules SET weekly_days='['||weekly_day||']' WHERE schedule_type='weekly' AND weekly_days IS NULL AND weekly_day IS NOT NULL;");
    for(const [name,definition] of [["email_enabled","INTEGER NOT NULL DEFAULT 0"],["email_to","TEXT"],["email_subject","TEXT"]] as const)if(!scheduleColumns.has(name))addColumn("schedules",name,definition);
    const occurrenceColumns=new Set((this.database.prepare("PRAGMA table_info(schedule_occurrences)").all() as {name:string}[]).map(column=>column.name));
    if(!occurrenceColumns.has("email_config_snapshot_json"))addColumn("schedule_occurrences","email_config_snapshot_json","TEXT");
    this.database.exec("CREATE TABLE IF NOT EXISTS schedule_email_outbox (id TEXT PRIMARY KEY,occurrence_id TEXT NOT NULL,notification_type TEXT NOT NULL DEFAULT 'job-result',config_snapshot_json TEXT NOT NULL,result_snapshot_json TEXT NOT NULL,attachment_content BLOB,attachment_name TEXT,attachment_encoding TEXT NOT NULL DEFAULT 'utf8',attachment_sha256 TEXT,status TEXT NOT NULL,attempts INTEGER NOT NULL DEFAULT 0,next_attempt_at TEXT,lease_owner TEXT,lease_expires_at TEXT,message_id TEXT,sent_at TEXT,error_code TEXT,redacted_error_message TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,UNIQUE(occurrence_id,notification_type)); CREATE INDEX IF NOT EXISTS idx_email_outbox_due ON schedule_email_outbox(status,next_attempt_at);");
    ...
    if (/conversation_id TEXT REFERENCES|run_id TEXT REFERENCES/i.test(occurrenceSql)) {
      this.database.exec(`PRAGMA foreign_keys=OFF; BEGIN; ALTER TABLE schedule_occurrences RENAME TO schedule_occurrences_old; CREATE TABLE schedule_occurrences (id TEXT PRIMARY KEY,schedule_id TEXT NOT NULL REFERENCES schedules(id) ON DELETE CASCADE,scheduled_for TEXT NOT NULL,status TEXT NOT NULL,conversation_id TEXT,run_id TEXT,started_at TEXT,ended_at TEXT,result TEXT,error_code TEXT,error_message TEXT,log_file TEXT,email_config_snapshot_json TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,UNIQUE(schedule_id,scheduled_for)); INSERT INTO schedule_occurrences (id,schedule_id,scheduled_for,status,conversation_id,run_id,started_at,ended_at,result,error_code,error_message,log_file,email_config_snapshot_json,created_at,updated_at) SELECT id,schedule_id,scheduled_for,status,conversation_id,run_id,started_at,ended_at,result,error_code,error_message,log_file,email_config_snapshot_json,created_at,updated_at FROM schedule_occurrences_old; DROP TABLE schedule_occurrences_old; CREATE INDEX IF NOT EXISTS idx_occurrences_schedule ON schedule_occurrences(schedule_id,scheduled_for); CREATE INDEX IF NOT EXISTS idx_occurrences_run ON schedule_occurrences(run_id); COMMIT; PRAGMA foreign_keys=ON;`);
    }
  }
